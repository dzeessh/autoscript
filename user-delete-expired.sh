...
if [ ! -f /usr/local/bin/deleteduser ]; then
    echo "echo "Script Created By DZEESSH.com"" > /usr/local/bin/deleteduser
...
for((i=1; i<=$totalaccounts; i++ ))
       ...
       echo "echo "DZEESSH.com- User : $username Date Expired On : $tgl $bulantahun"" >> /usr/local/bin/alluser
       todaystime=`date +%s`
